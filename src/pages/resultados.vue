<template>
    <div class="resultados">
      <v-row>
        <v-col cols="6">
          <br />
          <h2>Ganancias del ciclo {{ shop.currentCycle - 1 }}:</h2>
        ...
        <v-col cols="6">
          ...
          <div v-if="!nextPage">
            <v-row>
              ...
              <v-col cols="6">
                <h3>{{ shop.money - ultimoBeneficio.lastBenefits }} Lps</h3>
                <br />
                <h3>{{ ultimoBeneficio.lastBenefits }} Lps</h3>
                <br />
              </v-col>
              <v-row>
                <v-col cols="12">
                  <div class="beneficios elevation-6 d-flex pa-3">
                    ...
                    <v-col cols="9">
                      <h3 class="text-right">{{ shop.money }}</h3>
                    </v-col>
                  </div>
                </v-col>
              </v-row>
            ...
            <v-row>
              <v-col cols="12">
                <v-table height="300px" fixed-header class="elevation-6">
                  ...
                  <tbody>
                    <tr
                      v-for="product in shop.inventory"
                      :key="product._id"
                      class="text-left"
                    >
                      <td>juan</td>
  ...
  const traerInfo = async () => {
    ...
    (ultimoBeneficio.value = shopStore.ultimoBeneficio),
  ...
  const labels = cicloDatos.value.map((cycle) =>
    cycle.cycleNumber.toString()
  );
  const dataSet = cicloDatos.value.map((cycle) => {
    cycle.lastBenefits;
  });
